...
def parse_option():
    parser = argparse.ArgumentParser(
        "Mixed Vision Transformer training and evaluation script", add_help=False
    )
    parser.add_argument(
        "--cfg",
        type=str,
    ...
    parser.add_argument(
        "--pretrained", type=str, help="Finetune 384 initial checkpoint.", default=""
    )
...
def main():
    ...
    for epoch in range(config.TRAIN.START_EPOCH, total_epochs):
        ...
        if (
            dist.get_rank() == 0
            and ((epoch + 1) % config.SAVE_FREQ == 0 or (epoch + 1) == (total_epochs))
            and acc1 >= max_accuracy
        ):
            save_checkpoint_new(
                config,
                epoch + 1,
